//! Complete order processing system demonstrating all Ergon features with versioning.
//!
//! Run with
//!
//! ```not_rust
//! cargo run --example comprehensive_demo_3 --features=sqlite
//! ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let storage = Arc::new(SqliteExecutionLog::new("data/comprehensive_demo_3.db").await?);
    storage.reset().await?;
