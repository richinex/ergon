...
impl OrderProcessor {
    ...
    async fn reserve_inventory(
        ...
        println!("  [Step 3/4] Reserving inventory (attempt #{}) for transaction {}",
        ...
        let should_fail = INVENTORY_SHOULD_FAIL_ONCE.fetch_sub(1.min(INVENTORY_SHOULD_FAIL_ONCE.load(Ordering::SeqCst)), Ordering::SeqCst);
        if should_fail > 0 {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for inv in &invocations {
        if inv.step() > 0 {  // Skip flow itself (step 0)
            match inv.status() {
    ...
    println!("In production: Ergon would automatically retry the flow");
    println!("For demo purposes: We'll manually trigger retry\n");

    // Re-schedule the same order (in production: automatic retry would handle this)
    ...
    if payment_count == 1 {
        ...
        println!("  ✗ Payment step ran {} times - duplicate charge!", payment_count);
    }

    println!("\n💡 With Regular Queues:");
    println!("  ✗ Entire task would retry from step 1");
    println!("  ✗ Customer would be charged {} times",
