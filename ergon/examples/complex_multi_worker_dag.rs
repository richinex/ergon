...
//!
//! ```bash
//! cargo run --example complex_multi_worker_dag --features=sqlite
//! ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("\n╔════════════════════════════════════════════════════════════╗");
    println!("║ Complex Multi-Worker SEQUENTIAL + Child Flows (SQLite)    ║");
    println!("╚════════════════════════════════════════════════════════════╝\n");
    println!("Scenario: 3 concurrent orders, 4 workers, SEQUENTIAL execution (NO DAG)\n");

    let storage = Arc::new(SqliteExecutionLog::new("complex_dag.db").await?);
    storage.reset().await?;
    ...
    println!("\nWaiting 1 second before starting workers to ensure all orders are queued...\n");
