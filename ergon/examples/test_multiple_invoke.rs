//! Test: Can we have multiple sequential steps that each invoke a child?
//!
//! This tests whether the step-child mapping handles:
//! Step 2: invoke(PaymentFlow)   → ergon:step_child:flow_id:2
//! Step 3: invoke(ShipmentFlow)  → ergon:step_child:flow_id:3
...
// =============================================================================
// Parent Flow - Has TWO steps that each invoke a child
// =============================================================================
...
impl Order {
    ...
    async fn ship(self: Arc<Self>) -> Result<ShipmentResult, String> {
        ...
        println!("[{}] Step 3: got shipment result: {:?}", ts(), result);
        Ok(result)
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let db = "/tmp/ergon_test_multiple_invoke.db";
    let _ = std::fs::remove_file(db);

    let storage = Arc::new(SqliteExecutionLog::new(db).await?);
    let scheduler = Scheduler::new(storage.clone());

    println!("\n=== Test: Multiple Sequential Steps with .invoke() ===\n");
